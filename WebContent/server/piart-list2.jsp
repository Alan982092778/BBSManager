  ...
  <body>
    ...
    <div class="x-body">
      ...
      <xblock>
        ...
        <button class="layui-btn" onclick="x_admin_show('添加用户','${pageContext.request.contextPath}/server/piart-add.jsp',800,600)"><i class="layui-icon"></i>添加</button>
      ...
      <table class="layui-table x-admin">
        ...
        <tbody>
        	<c:forEach items="${list }" var="ls">
        		<tr>
		            <td>
		              <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='${ls.plateId }'><i class="layui-icon">&#xe605;</i></div>
		            </td>
		             <td>${ls.plateId }</td>
            		<td>${ls.plateTitle }</td>
            		<td>${ls.plateMessage }</td>
            		<td>${ls.isEnable }</td>
            		 <td class="td-status">
              <span class="layui-btn layui-btn-normal layui-btn-mini">可查看</span></td>
		            <td class="td-manage">
		              ...
		              <a title="编辑"  onclick="x_admin_show('编辑','${pageContext.request.contextPath}/PiartServelt?op=findId&plateId=${ls.plateId}',600,400)" href="javascript:;">
		                <i class="layui-icon">&#xe642;</i>
		              </a>
		              <!-- <a onclick="x_admin_show('修改密码','member-password.html',600,400)" title="修改密码" href="javascript:;">
		                <i class="layui-icon">&#xe631;</i>
		              </a> -->
		              <a title="删除" onclick="member_del(this,'${ls.plateId}')" href="javascript:;">
		                <i class="layui-icon">&#xe640;</i>
		              </a>
		            </td>
		          </tr>
    ...
    <script>
      ...
      function member_del(obj,id){
          layer.confirm('确认要删除吗？',function(index){
        	 $.post("${pageContext.request.contextPath }/PiartServelt?op=delete",{plateId:id},function(data){
        		 if(data.result=="true"){
        			//发异步删除数据
      ...
      function delAll () {
        ...
        layer.confirm('确认要删除吗？'+data,function(index){
        	$.ajax({
        		url:"${pageContext.request.contextPath}/PiartServelt?op=deleteAll",
