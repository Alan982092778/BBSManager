  ...
  <body>
    ...
    <div class="x-body">
      ...
      <xblock>
        ...
        <button class="layui-btn" onclick="x_admin_show('添加用户','${pageContext.request.contextPath}/server/invitation-add.jsp',800,600)"><i class="layui-icon"></i>添加</button>
      ...
      <table class="layui-table x-admin">
        ...
        <tbody>
        	<c:forEach items="${listi }" var="li">
        		<tr>
		            <td>
		              <div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='${li.invitationId }'><i class="layui-icon">&#xe605;</i></div>
		            </td>
		            <td>${li.invitationId }</td>
		            <td>${li.invitationMessage }</td>
		            <td>${li.userId }</td>
		            <td>${li.plateTitle }</td>
		            <td>${li.category }</td>
		            <td>${li.isPass }</td>
		            <td>${li.isEnable}</td>
		            <td>${li.isCream }</td>
		            <td>${li.invitationCreate }</td>
		            <td>${li.invitationModify }</td>
		            <td class="td-manage">
		            <a title="查看帖子详情"  onclick="x_admin_show('帖子详情','${pageContext.request.contextPath}/InvitationServerlt?op=details&inviId=${li.invitationId }')" href="javascript:;">
		              ...
		              <a title="编辑"  onclick="x_admin_show('编辑','${pageContext.request.contextPath}/InvitationServerlt?op=findId&invitationId=${li.invitationId }',600,400)" href="javascript:;">
		                <i class="layui-icon">&#xe642;</i>
		              </a>
		              <a title="删除" onclick="member_del(this,'${li.invitationId}')" href="javascript:;">
		                <i class="layui-icon">&#xe640;</i>
		              </a>
		            </td>
		          </tr>
    ...
    <script>
      ...
      function member_del(obj,id){
          layer.confirm('确认要删除吗？',function(index){
        	 $.post("${pageContext.request.contextPath }/InvitationServerlt?op=delete",{invitationId:id},function(data){
        		 if(data.result=="true"){
        			//发异步删除数据
